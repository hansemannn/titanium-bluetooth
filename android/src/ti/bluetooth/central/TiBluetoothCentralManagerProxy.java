...
public class TiBluetoothCentralManagerProxy extends KrollProxy implements OnBluetoothStateChangedListener, OnPeripheralConnectionStateChangedListener {
    ...
    private static final int SCAN_MODE_BALANCED = TiBluetoothModule.SCAN_MODE_BALANCED;
    private static final int SCAN_MODE_LOW_LATENCY = TiBluetoothModule.SCAN_MODE_LOW_LATENCY;
    private static final int SCAN_MODE_LOW_POWER = TiBluetoothModule.SCAN_MODE_LOW_POWER;
    private static final int SCAN_MODE_OPPORTUNISTIC = TiBluetoothModule.SCAN_MODE_OPPORTUNISTIC;
    ...
    @Kroll.method
    public void connectPeripheral(TiBluetoothPeripheralProxy peripheral, KrollDict properties) {
        boolean notifyOnConnection = properties.getBoolean(NOTIFY_ON_CONNECTION_KEY);
        boolean notifyOnDisconnection = properties.getBoolean(NOTIFY_ON_DISCONNECTION_KEY);

        peripheral.connectPeripheral(context, notifyOnConnection, notifyOnDisconnection, this);
    }

    @Override
    public void onPeripheralConnectionStateConnected(TiBluetoothPeripheralProxy peripheral) {
        fireCentralManagerEvent(DID_CONNECT_PERIPHERAL,peripheral);
    }

    @Override
    public void onPeripheralConnectionStateDisconnected(TiBluetoothPeripheralProxy peripheral) {
        fireCentralManagerEvent(DID_DISCONNECT_PERIPHERAL, peripheral);
    }

    @Override
    public void onPeripheralConnectionStateError(TiBluetoothPeripheralProxy peripheral) {
        fireCentralManagerEvent(DID_FAIL_TO_CONNECT_PERIPHERAL, peripheral);
    }

    @Kroll.method
    public void cancelPeripheralConnection(TiBluetoothPeripheralProxy peripheral) {
    ...
    public void setScanMode(int scanMode) {
        if (scanMode == SCAN_MODE_BALANCED || scanMode == SCAN_MODE_LOW_LATENCY || scanMode == SCAN_MODE_LOW_POWER || scanMode == SCAN_MODE_OPPORTUNISTIC) {
    ...
    private void processResult(ScanResult result) {
        TiBluetoothPeripheralProxy bluetoothPeripheral = new TiBluetoothPeripheralProxy(result.getDevice());
        fireCentralManagerEvent(DID_DISCOVER_PERIPHERAL, bluetoothPeripheral);
    }

    private void fireCentralManagerEvent(String event, TiBluetoothPeripheralProxy bluetoothPeripheral) {
